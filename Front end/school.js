function fun(){
    ...
    .then((res)=>{
        ...
        if(res.data[i].district==dis && res.data[i].block==block){
        if(!set.has(res.data[i].name)){
        ...
        <div class="col-lg-4">
            ...
            <input type="button" class="btn btn-primary" placeholder="Block"  value="${res.data[i].name}" onclick="order(this)" style="color:dark;" >
            </div>
            </div>
        <div>
        `}}}
...
function order(objButton){
    sessionStorage.setItem("school",objButton.value)
    window.open('school1.html','_self');
}